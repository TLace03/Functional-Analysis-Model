...
# ============================================================
# Imports NewsAgent.py (must be in the same directory).
# The signal only affects dates within LIVE_SIGNAL_WINDOW_DAYS of today,
# so historical out-of-sample backtest results are never contaminated.
# If NewsAgent.py is absent or any dependency fails, the model runs
# exactly as before — the news layer is strictly additive.
try:
...
def resolve_phase(date, base_phase):
    ...
    News signal layer (live dates only):
    If _news_signal is available and the date falls within
    LIVE_SIGNAL_WINDOW_DAYS of today, NewsSignal.adjust_phase()
    is called after the momentum check and may further override
    the phase based on Polymarket recession pricing or news
    sentiment. Historical backtest dates always pass through
    adjust_phase() as a no-op — backtest integrity is preserved.
    """
    if base_phase == 1:
...
for date in test_returns_raw.index:
    ...
    # News signal: marginal blend nudges for live/recent dates only.
    # adjust_blend() is a no-op for historical dates (backtest safe).
    if _news_signal is not None:
